...
public class ShadowMapMasterRenderer {
	...
	private static final int SHADOW_MAP_SIZE = 4096; // send this to terrain fragment shader
	 ...
	 */
	public void render(Map<TexturedModel, List<Entity>> entities, Map<TexturedModel, List<Entity>> normalMapEntities, Light sun) {
		shadowBox.update();
		Vector3f sunPosition = sun.getPosition();
		Vector3f lightDirection = new Vector3f(-sunPosition.x, -sunPosition.y, -sunPosition.z);
		prepare(lightDirection, shadowBox);
		
		Map<TexturedModel, List<Entity>> all = new HashMap<TexturedModel, List<Entity>>();
		all.putAll(entities);
		all.putAll(normalMapEntities);
		
		entityRenderer.render(all);
