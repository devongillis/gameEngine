...
private RawModel generateTerrain(Loader loader) {
	...
	private RawModel generateTerrain(Loader loader, String heightMap) {
		...
		for (int i = 0; i < VERTEX_COUNT; i++) {
			for (int j = 0; j < VERTEX_COUNT; j++) {
				vertices[vertexPointer * 3] = (float) j / ((float) VERTEX_COUNT - 1) * SIZE;
				float height = getHeight(j, i, /*image*/ generator);
				vertices[vertexPointer * 3 + 1] = height;
				heights[j][i] = height;
				vertices[vertexPointer * 3 + 2] = (float) i / ((float) VERTEX_COUNT - 1) * SIZE;
				Vector3f normal = calculateNormal(j, i, /*image*/ generator);
				normals[vertexPointer * 3] = normal.x;
				normals[vertexPointer * 3 + 1] = normal.y;
				normals[vertexPointer * 3 + 2] = normal.z;
				textureCoords[vertexPointer * 2] = (float) j / ((float) VERTEX_COUNT - 1);
	...
	private Vector3f calculateNormal(int x, int z, /*BufferedImage image*/ HeightGenerator generator){
	...
	private float getHeight(int x, int z, /*BufferedImage image,*/ HeightGenerator generator){
		/*
